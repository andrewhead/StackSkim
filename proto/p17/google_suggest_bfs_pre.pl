...
sub main() {
  ...
  while ($#seeds > -1) {
    ...
    ######
    my @results = &google_suggest("$seed$suffix", length($seed));

    print STDERR "'$seed$suffix' ". length($seed) . ": " . ($#results + 1) . " results\n";
    ...
    foreach my $result (@results) {
      ...
      print length($r{'query'}) - length($suffix) . ', ' .
            $r{'query'} . ', '.
               $r{'cp'} . ', '.
             $r{'rank'} . ', '.
           $r{'result'} . ', '.
